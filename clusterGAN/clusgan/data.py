...
LOAD DATA from file.
"""
from clusgan.definitions import *
...
## original version is from ganomaly
def load_data(dataset_name='mnist', ano_class=0, batchsize=64, workers=1,proportion=0.8):
    """ Load Data
    ...
    if dataset_name in ['cifar10']:
        ...
        dataset = {}
        dataset['train'] = MNIST(root=data_dir, train=True, download=True, transform=transform)
        dataset['test'] = MNIST(root=data_dir, train=False, download=True, transform=transform)
        ...
        dataloader = {x: torch.utils.data.DataLoader(dataset=dataset[x],
                                                     ...
                                                     num_workers=int(workers),
                                                     drop_last=drop_last_batch[x],
                                                     worker_init_fn=(None if manualseed == -1
                                                     else lambda x: np.random.seed(manualseed)))
                      for x in splits}
        return dataloader
        ...
        dataset = {x: ImageFolder(os.path.join(RUNS_DIR, x), transform) for x in splits}
        dataloader = {x: torch.utils.data.DataLoader(dataset=dataset[x],
                                                     batch_size=batchsize,
                                                     shuffle=shuffle[x],
                                                     num_workers=int(workers),
