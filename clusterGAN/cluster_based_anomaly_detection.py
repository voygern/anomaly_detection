...
try:
    ...
    import clusgan.vgg as models
    import clusgan.data as anomaly_data
    from clusgan.evaluate import evaluate
...
def main():
    ...
    parser.add_argument("-r", "--run_name", dest="run_name", default='ano', help="Name of training run")
    ...
    # load anomaly train data and test data
    dl = anomaly_data.load_data(ano_class=9, batchsize=batch_size)
    ...
    optimizer_C = torch.optim.Adam(classifier.parameters(), lr=lr, weight_decay=0.0)
    ...
    for epoch in range(n_epochs):
        for i, (imgs, itruth_label) in enumerate(dataloader):
            ...
            if (i % n_skip_iter == 0):
                ...
                # Add by voygern encode the real images
                # enc_real_zn, enc_real_zc, enc_real_zc_logits = encoder(real_imgs)

                # Calculate losses for cluster
                # cluster_loss = xe_loss(enc_real_zc, enc_real_zc.argmax(dim=1))
                ...
                if wass_metric:
                    ...
                    ge_loss = torch.mean(D_gen) + betan * zn_loss + betac * zc_loss #+ 10 * cluster_loss

                else:
                    # Vanilla GAN loss
                    valid = Variable(Tensor(gen_imgs.size(0), 1).fill_(1.0), requires_grad=False)
                    v_loss = bce_loss(D_gen, valid)
                    ge_loss = v_loss + betan * zn_loss + betac * zc_loss #+ 10 * cluster_loss
            ...
            normal_data = Variable(imgs.type(Tensor))
            enc_real_zn, enc_real_zc, enc_real_zc_logits = encoder(normal_data.data)
            soft_label = enc_real_zc.argmax(dim=1)
            # normal_data = normal_data.reshape(normal_data.size(0), 1, normal_data.size(1), normal_data.size(2))
            real_c = F.softmax(classifier(normal_data) / temprature, dim=1)
            loss_real_c = xe_loss(real_c, soft_label)

            # Calculate loss for abnormal data
            i_fake = 1 - discriminator(gen_imgs.data).round()
            abnormal_data = Variable(gen_imgs[i_fake > 0.5].data)
            uni_label = Tensor(abnormal_data.size(0), 10).fill_(1. / 10)
            abnormal_c = F.softmax(classifier(abnormal_data.unsqueeze(dim=1)) / temprature, dim=1)
            loss_abnormal_c = cross_entropy(abnormal_c, uni_label)

            loss_classifier = loss_real_c + loss_abnormal_c
    ...
    model_list = [discriminator, encoder, generator, classifier]
    ...
    with torch.no_grad():
        ...
        an_scores = torch.zeros(size=(len(dl['test'].dataset),), dtype=torch.float32)
        gt_labels = torch.zeros(size=(len(dl['test'].dataset),), dtype=torch.long)
        ...
        total_steps = 0
        epoch_iter = 0
        for i, (img,lbl) in enumerate(dl['test'], 0):
            total_steps += batchsize
            epoch_iter += batchsize
            time_i = time.time()

            c_out = classifier(img)

            error = torch.max(F.softmax(c_out, dim=1), dim=1)
            time_o = time.time()

            an_scores[i*batchsize: i*batchsize+error.size(0)] = error.reshape(error.size(0))
            gt_labels[i*batchsize: i*batchsize+error.size(0)] = lbl.reshape(error.size(0))
        ...
        auc = evaluate(gt_labels, an_scores, metric=test_metric)
